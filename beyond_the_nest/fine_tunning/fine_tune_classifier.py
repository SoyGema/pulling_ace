...
def main():  # noqa: C901
    ...
    if is_regression:
        ...
        print("THIS IS THE LABLE LIST" + str(label_list))
    ...
    if training_args.do_train and not is_regression:  # classification, training
        ...
        print("LABEL_TO_ID IN DO TRAIN" + str(label_to_id))
    ...
    def preprocess_function(examples):
        ...
        # Introduce falcon logic in the script
        ...
        # Check if label_to_id is None
        if label_to_id is None:
            print("Warning: label_to_id is None, no label filtering will be applied.")

        # Check if "label" is not in examples
        if "label" not in examples:
            print(
                "Warning: 'label' key not found in examples, no label filtering will be applied."
            )

        # Check the unique labels in examples before filtering
        if "label" in examples:
            unique_labels_before = set(examples["label"])
            print(f"Unique labels before filtering: {unique_labels_before}")

        if label_to_id is not None and "label" in examples:
            filtered_indices = [
                i for i, label in enumerate(examples["label"]) if label in {0, 1}
            ]

            for key in result:
                result[key] = [result[key][i] for i in filtered_indices]

        # Check the unique labels in result after filtering (if applicable)
        if "label" in result:
            unique_labels_after = set(result["label"])
            print(f"Unique labels after filtering: {unique_labels_after}")

            if is_multi_label:
                result["label"] = [multi_labels_to_ids(i) for i in examples["label"]]
            else:
                result["label"] = [
                    (label_to_id[str(i)] if i != -1 else -1) for i in examples["label"]
                ]

        # Validate that only two labels (0 and 1) are present
        unique_output_labels = set(result["label"])
        if unique_output_labels.difference({0, 1}):
            raise ValueError(
                f"Unexpected output labels found: {unique_output_labels.difference({0, 1})}"
            )
